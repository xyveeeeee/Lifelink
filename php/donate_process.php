...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // PREVENT'S USERT DUPLICATIONS OF ORGAN DONATION 
    if ($donation_type === 'organ' && !empty($organ_type)) {
        $check_sql = "SELECT COUNT(*) 
                      FROM donations 
                      WHERE user_id = ? 
                      AND LOWER(donation_type) = 'organ' 
                      AND LOWER(organ_type) = LOWER(?) 
    ...
    if ($donation_type === 'blood' && !empty($blood_type)) {
        ...
        $check_blood_sql = "SELECT MAX(availability_date)
                            FROM donations
                            WHERE user_id = ?
                            AND LOWER(donation_type) = 'blood'
                            AND LOWER(blood_type) = LOWER(?)
                            AND status IN ('Pending', 'Approved', 'Completed')";
    ...
    $sql = "INSERT INTO donations 
            (user_id, donation_type, organ_type, blood_type, volume, hospital, availability_date, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
